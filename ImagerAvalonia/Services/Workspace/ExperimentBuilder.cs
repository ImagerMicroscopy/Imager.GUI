...
public class ExperimentBuilder : IDisposable {
    ...
    /// <summary>
    /// Sets the UI root node when loading an experiment from file.
    /// This synchronizes the loaded UI tree with the internal state tree.
    /// </summary>
    public void SetUiRootNode(RootNode rootNode) {
        // Clear existing state
        _uiRootNode = rootNode;
        _elementIdToNodeMap.Clear();
        _elementIdToStateMap.Clear();
        _programRoot = null;
        
        // Register all nodes in the loaded tree
        RegisterNode(rootNode);
        
        // Create corresponding state elements for all nodes
        foreach (var child in rootNode.Children) {
            CreateStateElement(child);
        }
        
        // Set the program root from the first child (if it exists)
        if (rootNode.Children.Count > 0 && rootNode.Children[0].NodeViewModel is MeasurementViewModel mv) {
            _programRoot = _elementIdToStateMap.GetValueOrDefault(mv.Elementid);
        }
        
        // Set up ExperimentBuilder reference on all ViewModels
        SetupViewModelReferences(rootNode);
        
        // Fire events to notify that the tree has been loaded
        RootNodeChanged?.Invoke(_uiRootNode);
        ConfigurationChanged?.Invoke(this, EventArgs.Empty);
    }
    
    /// <summary>
    /// Recursively sets up ExperimentBuilder references on all MeasurementViewModels in the tree.
    /// </summary>
    private void SetupViewModelReferences(NodeBase node) {
        if (node.NodeViewModel is MeasurementViewModel mv) {
            mv.ExperimentBuilder = this;
        }
        
        foreach (var child in node.Children) {
            SetupViewModelReferences(child);
        }
    }
    ...
    private void CreateStateElement(NodeBase node) {
        ...
        if (stateElement != null) {
            ...
            if (stateElement.IsContainerElement() && node is ExperimentNode expNode) {
                ...
                // We've processed the children recursively above, so we can return early
                return;
            }
        }
